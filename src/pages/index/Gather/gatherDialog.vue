<template>
  <el-dialog :title="item.id?'编辑分类':'新建分类'" :visible.sync="dialogVisibie" width="40%" :before-close="handleClose" center
...
<script>
  ...
  export default {
    ...
    methods: {
      upload(data) {
        console.log(data)
        this.$set(this.model, "img", data.url);
      },
      handleClose() {
      ...
      save() {
        ...
        this.item.id ? this.editGather() : this.addGather();
      },
      async addGather() {
        let {
          data
